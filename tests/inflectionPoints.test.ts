...
describe("inflectionPoints", () => {
  describe("detectInflectionPoints", () => {
    ...
    it("should detect multiple peaks and valleys", () => {
      ...
      expect(points.filter((p) => p.type === "peak").length).toBeGreaterThan(0);
      expect(points.filter((p) => p.type === "valley").length).toBeGreaterThan(0);
    });
  ...
  describe("detectInflectionPointsWithDates", () => {
    it("should attach dates to inflection points", () => {
      const data = [
        { value: 1, date: new Date("2026-01-01") },
        { value: 2, date: new Date("2026-01-02") },
        { value: 3, date: new Date("2026-01-03") },
        { value: 2, date: new Date("2026-01-04") },
        { value: 1, date: new Date("2026-01-05") },
      ];
      const points = detectInflectionPointsWithDates(data, 1);

      expect(points).toHaveLength(1);
      expect(points[0].date).toEqual(new Date("2026-01-03"));
    });
  });
